...
The NIX Service is a Node.js interface to [NIX Core](https://github.com/NixPlatform/nix-core) for querying information about the nix block chain. It will manage starting and stopping `nixd` or connect to several running `nixd` processes. It uses an instance of a NIX with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "nixd". This defines the location of the block chain database and the location of the `nixd` daemon executable. The below configuration points to a local clone of `nix`, and will start `nixd` automatically with your Node.js application.
...
It's also possible to connect to separately managed `nixd` processes with round-robin quering, for example:
